...
const signupUser =async function(req,res){
    ...
    if (allKeys[i] != keUser[i])
      return res
        ...
        .send({
          status: false,
          message:
            "all fields mandatory , firstName,lastName ,subject,marks,mobile,password",
        });
  }

  if (isValidBody) {
    res.status(400).send({status:false,message:"please provide body"})
  }
  ...
  if(!isValidName(firstName)){
   ...
   if (!isValidPhone(mobile)) {
     return res
       .status(500)
       .send({ status: false, message: "please enter valid mobile no   " });
   }
