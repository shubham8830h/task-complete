  ...
  try {
    ...
    if (CheckUser) {
      return res
        ...
        .send({
          status: false,
          message: "you are already registered please go to log in page ",
        });
    }
...
const logInAdmin = async function (req, res) {
  try {
    ...
    if (isValidBody(req.body)) {
      return res
        .status(400)
        .send({ status: false, msg: "please input email and password" });
    }
    ...
    if (CheckUser) {
      ...
      return res
        .status(201)
        .send({ status: true, message: "logIn successfully", message: token });
    } else {
      return res
        .status(400)
        .send({ status: false, message: " please register before login in " });
    }
  } catch (error) {
    res.status(500).send({ status: false, message: error.message });
  }
